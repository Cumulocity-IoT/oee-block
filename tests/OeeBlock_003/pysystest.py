...
class PySysTest(AnalyticsBuilderBaseTest):
	...
	def execute(self):
		...
		self.sendEventStrings(correlator,
							  ...
							  self.inputEvent('pieces_ok', 2, id=modelId),
                              self.timestamp(70),
                              self.inputEvent('pieces_nok', 1, id=modelId),
							  self.inputEvent('pieces_ok', 2, id=modelId),
							  self.timestamp(110),
	...
	def validate(self):
		...
		self.assertBlockOutput('performance', 	[0.75,		0.45,	0.45,	0.45,	0.225,	 	0.675, 		0.5])
		self.assertBlockOutput('quality', 		[0.6667,	0.6667,	0.6667,	0.6667,	0.6667,	 	0.6667, 	0.6667])
		self.assertThat('output == expected', 
						output=self.details('ActualProductionAmount'), 
						expected=				[7.5, 		4.5, 	4.5, 	4.5, 	1.5, 		4.5, 		5.0])
