...
class PySysTest(OeeBaseTest):
	...
	def execute(self):
		...
		modelId = self.createTestModel('apamax.analyticsbuilder.oee.Oee', 
								 inputs={'status':'boolean', 'amount':'float', 'amount_ok':None ,'amount_nok':'float','qok':None},
		...
		self.sendEventStrings(correlator,
							  ...
							  self.timestamp(450),
							  self.inputEvent('amount', 10, id=modelId),
							  self.inputEvent('amount_nok', 5, id=modelId),
							  self.timestamp(510),
							  self.inputEvent('amount', 10, id=modelId),
							  self.inputEvent('amount_nok', 5, id=modelId),
							  # Performance=50
							  ...
							  self.inputEvent('status', True, id=modelId),
							  self.inputEvent('amount', 5, id=modelId),
							  self.inputEvent('amount_nok', 2.5, id=modelId),
							  self.timestamp(840),
							  self.inputEvent('status', False, id=modelId),
							  self.timestamp(870),
							  self.inputEvent('status', True, id=modelId),
							  ...
							  self.inputEvent('status', True, id=modelId),
							  self.inputEvent('amount', 2.5, id=modelId),
							  self.inputEvent('amount_nok', 1.25, id=modelId),
							  self.timestamp(1080),
							  self.inputEvent('status', False, id=modelId),
							  self.timestamp(1110),
							  self.inputEvent('status', True, id=modelId),
							  ...
							  self.timestamp(1170),
							  self.inputEvent('amount', 10, id=modelId),
							  self.inputEvent('amount_nok', 10, id=modelId),
                              self.timestamp(1230),
							  self.inputEvent('amount', 10, id=modelId),
							  self.inputEvent('amount_nok', 10, id=modelId),
