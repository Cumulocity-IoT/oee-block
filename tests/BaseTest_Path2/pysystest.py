...
class PySysTest(OeeBaseTest):
	...
	def execute(self):
		...
		modelId = self.createTestModel('apamax.analyticsbuilder.oee.Oee', 
								 inputs={'status':'boolean', 'amount':'float', 'amount_ok':None ,'amount_nok':None,'qok':'boolean'},
		...
		self.sendEventStrings(correlator,
							  ...
							  self.timestamp(1005),
							  self.inputEvent('qok', True, id=modelId),							  
							  self.inputEvent('amount', 1.25, id=modelId),
							  ...
							  self.inputEvent('qok', True, id=modelId),
							  self.inputEvent('status', True, id=modelId),
							  self.inputEvent('amount', 0.0, id=modelId),
							  self.timestamp(1065),
							  self.inputEvent('qok', False, id=modelId),
							  self.inputEvent('amount', 1.25, id=modelId),
							  self.timestamp(1080),
